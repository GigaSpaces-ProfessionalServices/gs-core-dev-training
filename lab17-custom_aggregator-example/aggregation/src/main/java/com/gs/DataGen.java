...
public class DataGen {
    ...
    final static int N_CURSES= 50000;
    ...
    public void writeData(GigaSpace gs){
        ...
        ArrayList<StudentCourses> studentCourses = new ArrayList<>(N_CURSE_PER_STUDENT);
        for (int k=1; k<= N_STUDENTS; k++){
            for (int c=1; c<=N_CURSE_PER_STUDENT; c++){
                studentCourses.add(new StudentCourses(k,c));
            }
            gs.writeMultiple(studentCourses.toArray());
            studentCourses = new ArrayList<>(N_CURSE_PER_STUDENT);
        }
    }
